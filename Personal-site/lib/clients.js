// lib/clients.js
//
// Shared helper for listing + normalizing client-work photos from Bunny Storage.
// Used by both /api/clients (JSON endpoint) and /api/client-page (server-rendered
// page). Mirrors lib/photos.js but reads from the clients/ subfolder and groups
// by "project" (client/project name) instead of "category".
//
// Required environment variables (shared with lib/photos.js):
...
export async function getClientPhotos() {
  ...
  const listUrl = `https://${host}/${zone}/${FOLDER}/`;
  ...
  const [bunnyRes, metaRes] = await Promise.all([
    ...
    fetch(`${pullZone}/${FOLDER}/clients.json`, { cache: 'no-store' })
      .then(r => r.ok ? r.json() : [])
      .catch(() => [])
  ]);
  ...
  const photos = files
    ...
    .filter(f => f.ObjectName !== 'clients.json')
    ...
    .map(f => {
      ...
      return {
        ...
        url:         `${pullZone}/${FOLDER}/${f.ObjectName}`,
        title:       meta.title || '',
